...
function appHandleUniform()
{
 ...
 $(".datetimepicker").datetimepicker({
  ...
  $('.colorpicker-default').colorpicker().on('hide', function(ev){    
    $('input',this).val(ev.color.toHex().replace('#',''));
...
{    
  if($('#form_tickets_types_id').length)
  {
...
function userPattern(id,field_id)
{
  ...
  if(desc.length>0)
  {
    $('#'+field_id).html($('#'+field_id).val()+desc)
  }
  else
  {
    $('#'+field_id).html($('#'+field_id).val()+name)
  }

  if(CKEDITOR_holders[field_id])
  {
    CKEDITOR_holders[field_id].setData( $('#'+field_id).val(),function() {
    this.focus();        

      var s = this.getSelection(); // getting selection
...
function show_original_text(id,v)
{
  ...
  if(CKEDITOR)
  {
    CKEDITOR_holders[id].setData( $('#'+id).val() );
  }
}
...
function check_users_groups_items_access()
{
  items = new Array('users_groups_allow_manage_projects','users_groups_allow_manage_tasks','users_groups_allow_manage_tickets','users_groups_allow_manage_discussions');
  
  for(i=0;i<items.length;i++)
  {      
    //check if there is custom access
    if($('#'+items[i]).val()=='custom')
    {
      $('#'+items[i]+'_custom').css('display','table');
    }
    else
    {
      $('#'+items[i]+'_custom').css('display','none');
    }
    
    //check if there is access
    if($('#'+items[i]).val()!='')
    {
      $('#'+items[i]+'_extra').css('display','table');
    }
    else
    {
      $('#'+items[i]+'_extra').css('display','none');
      
      switch(items[i])
      {
        case 'users_groups_allow_manage_projects':
            $('#users_groups_projects_comments_access').attr('checked',false);
            $('#users_groups_allow_manage_reports_projects').attr('checked',false);
            $('#users_groups_allow_manage_projects_wiki').val('');
            $('#users_groups_projects_custom_access_view_own_only').attr('checked',false);
            $('#users_groups_projects_custom_access_create').attr('checked',false);
            $('#users_groups_projects_custom_access_edit').attr('checked',false);
            $('#users_groups_projects_custom_access_delete').attr('checked',false);            
          break;
        case 'users_groups_allow_manage_tasks':
            $('#users_groups_tasks_comments_access').attr('checked',false);
            $('#users_groups_allow_manage_reports_tasks').attr('checked',false);
            $('#users_groups_allow_manage_reports_gantt').attr('checked',false);
            $('#users_groups_allow_manage_reports_time').attr('checked',false);
            $('#users_groups_allow_manage_reports_time_personal').attr('checked',false);
            $('#users_groups_tasks_custom_access_view_own_only').attr('checked',false);
            $('#users_groups_tasks_custom_access_create').attr('checked',false);
            $('#users_groups_tasks_custom_access_edit').attr('checked',false);
            $('#users_groups_tasks_custom_access_delete').attr('checked',false);            
          break;
        case 'users_groups_allow_manage_tickets':
            $('#users_groups_tickets_comments_access').attr('checked',false);
            $('#users_groups_allow_manage_reports_tickets').attr('checked',false);            
            $('#users_groups_tickets_custom_access_view_own_only').attr('checked',false);
            $('#users_groups_tickets_custom_access_create').attr('checked',false);
            $('#users_groups_tickets_custom_access_edit').attr('checked',false);
            $('#users_groups_tickets_custom_access_delete').attr('checked',false);            
          break;
        case 'users_groups_allow_manage_discussions':
            $('#users_groups_discussions_comments_access').attr('checked',false);
            $('#users_groups_allow_manage_reports_discussions').attr('checked',false);            
            $('#users_groups_discussions_custom_access_view_own_only').attr('checked',false);
            $('#users_groups_discussions_custom_access_create').attr('checked',false);
            $('#users_groups_discussions_custom_access_edit').attr('checked',false);
            $('#users_groups_discussions_custom_access_delete').attr('checked',false);            
          break;  
...
function use_related_comments(id,field_id)
{   
  desc = '<blockquote>'+$('#related_ticket_comment_'+id).val()+'</blockquote>';
  
  //alert(desc);
  
  if(CKEDITOR_holders[field_id])
  {
    CKEDITOR_holders[field_id].insertHtml( desc); 
  }
}
