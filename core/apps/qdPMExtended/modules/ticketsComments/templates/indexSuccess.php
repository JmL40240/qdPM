...
<div class="row">
    <div class="col-md-8 tickets-info">
...
<table>
  ...
  <?php endif ?>
    ...
    <div id="extraFieldsInDescription"><?php echo ExtraFieldsList::renderDescriptionFileds('tickets',$tickets,$sf_user,$tickets->getTicketsTypes()->getExtraFields()) ?></div>
...
<table class="table table-striped table-bordered table-hover" id="comments-table">
  ...
  <tbody>
    ...
    <tr>
      <td>
        <?php 
         ...
         /* if($comments_access['edit'] and $comments_access['view_own'])
          ...
          {
            echo $lc->action_buttons($c['id']) . '<br>' . $lc->copy_button($c['id']);
          }*/
           ?>
      </td>            
      ...
      <td><?php //echo (strlen($c['message_id'])>0 ? $c['user_name'] . '<br>' . $c['user_email'] : app::dateTimeFormat($c['created_at']) . '<br>' . $c['Users']['name'] . '<br>' .renderUserPhoto($c['Users']['photo'])) ?></td>      
    </tr>
    <?php //endforeach; ?>
...
<div><?php //if($comments_access['insert']) echo $lc->insert_button(__('Add Comment')); ?><br><br></div>
-->
<?php endif ?>      
    </div>
