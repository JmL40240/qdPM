<?php
  ...
  $extra_fields = ExtraFieldsList::getFieldsByType('discussions',$sf_user,true,array('all'=>true));
  $extra_fields_all = ExtraFieldsList::getFieldsByType('discussions',$sf_user,false,array('all'=>true));
  $totals = array();
  
  $is_filter = array();
  $is_filter['priority'] = app::countItemsByTable('DiscussionsPriority');
  $is_filter['group'] = app::countItemsByTable('DiscussionsGroups');
...
<table class="table table-striped table-bordered table-hover discussions-table" id="itmes_listing_<?php echo $tlId ?>" <?php echo (count($discussions_list)>0 ? 'style="display:none"':'')?> >
  ...
  <tbody>
    ...
    <tr>
      ...
      <?php if($is_filter['priority']): ?>
      <td><?php echo app::getArrayNameWithIcon($discussions,'DiscussionsPriority') ?></td>
      <?php endif; ?>
      ...
      <?php if($is_filter['group']): ?>
      <td><?php echo app::getArrayName($discussions,'DiscussionsGroups') ?></td>
      <?php endif; ?>
                  
      <td><?php echo $discussions['Users']['name'] ?></td>
      <td><?php echo app::dateFormat($discussions['created_at'],0,true) ?></td>
                                                
      <?php
        $v = ExtraFieldsList::getValuesList($extra_fields_all,$discussions['id']); 
