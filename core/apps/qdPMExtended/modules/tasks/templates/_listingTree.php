<?php
  ...
  $o = Doctrine_Core::getTable('UsersListingsOrder')
          ...
          ->addWhere('projects_id=?',$sf_request->getParameter('projects_id'))
          ->fetchOne();
    ...
    <td>
      <table>
        <tr>
          ...
          <?php endif ?>
          <td style="padding-right: 15px;"><?php  echo $lc->sort_tasks_tree(__('Sort Tree')) ?></td>
...
<table class="table table-striped table-bordered  table-hover  tableListing withoutPadding" id="tasksTreeListing" >
  <thead>
    <tr>
      <?php echo $lc->rednerMultipleSelectTh() ?>
      <?php echo $lc->rednerActionTh() ?>      
      
      <?php if(in_array('Id',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Id') ?></div></th>
      <?php endif; ?>
      
      <?php if(in_array('TasksGroups',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Group') ?></div></th>
      <?php endif; ?>
      
      <?php if(in_array('Versions',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Version') ?></div></th>
      <?php endif; ?>
      
      <?php if(in_array('ProjectsPhases',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Phase') ?></div></th>
      <?php endif; ?>
      
      <?php if(in_array('TasksPriority',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Priority') ?></div></th>
      <?php endif; ?>
            
      <?php if(in_array('TasksTypes',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Type') ?></div></th>
      <?php endif; ?>
                         
	  <?php if(in_array('TasksStatus',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Status') ?></div></th>  
      <?php endif; ?>
	  
	  <?php if(in_array('StartDate',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Start Date') ?></div></th>
      <?php endif; ?>
      
      <?php if(in_array('DueDate',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Due Date') ?></div></th>
      <?php endif; ?>
	  
	  <?php if(in_array('AssignedTo',$in_listing)):$cols++; ?>            
      <th><div><?php echo __('Assigned To') ?></div></th>
      <?php endif; ?>
						 
      <?php if(in_array('Name',$in_listing)): $cols++; ?>
      <th width="100%"><div><?php  __('Name') ?></div></th>
      <?php endif; ?>     
      
      <?php if(in_array('TasksLabels',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Label') ?></div></th>
      <?php endif; ?>   
      
      <?php if(in_array('CreatedBy',$in_listing)): $cols++; ?>            
      <th><div><?php echo __('Created By') ?></div></th>
      <?php endif; ?>
      
      <?php if(in_array('EstimatedTime',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Est. Time') ?></div></th>
      <?php endif; ?>
      
      <?php if(in_array('WorkHours',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Work Hours') ?></div></th>
      <?php endif; ?>
      
      <!--<?php //if(in_array('TogoHours',$in_listing)):  ?>
      <th><div><?php //echo __('Hours To Go') ?></div></th>
      <?php //endif; ?>-->
      
      <?php if(in_array('ActualTime',$in_listing)):  ?>
      <th><div><?php echo __('Actual Time') ?></div></th>
      <?php endif; ?>
	  
      <?php if(in_array('Progress',$in_listing)): $cols++; ?>
      <th><div><?php echo __('Progress') ?></div></th>
      <?php endif; ?>
      
      <?php if(in_array('CreatedAt',$in_listing)): $cols++; ?>
  ...
  <tbody>  
    ...
    <tr <?php echo  TasksTree::getTasksTreeTrClasses($tasks_tree, $tasks['id']) ?>>     
      <?php echo $lc->rednerMultipleSelectTd($tasks['id']) ?>
      <?php echo $lc->renderActionTd($tasks['id'],(!$sf_request->hasParameter('projects_id') ? $tasks['projects_id']:0),true,__('Are you sure you want to delete task') . '\n' . $tasks['name'] . '?') ?>      
      ...
      <td><?php echo app::dateTimeFormat($tasks['start_date']) ?></td>
      <?php endif; ?>
      
      <?php if(in_array('DueDate',$in_listing)): ?>
      <td><?php echo app::dueDateFormat($tasks['due_date']) ?></td>
      <?php endif; ?>
      ...
      <td>
        ...
        <table>
          <tr>
            <td><?php echo  TasksTree::getTasksTreeTdPadding($tasks_tree,$tasks['id'],$collapsed_tasks) ?></td>
            <td>         
      ...
      <td><?php echo app::dateTimeFormat($tasks['created_at']) ?></td>
      <?php endif; ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    jQuery('#tasksTreeListing tbody tr .checkboxes').change(function(){
    ...
    jQuery('#tasksTreeListing .group-checkable').change(function () {      
                
          var checked = jQuery(this).is(":checked");
          selected_items.length = 0;
          
          jQuery( "#tasksTreeListing .checkboxes").each(function(){                  
               if(checked)
               {                        
                  selected_items.push($(this).attr('value'));   
  ...
  function expandChildTasks(id)
  {
      if($('#folder'+id).attr('rel')=='collapsed')
      {
        $( ".taskRow"+id ).each(function() { $(this).removeClass('hiddenRow'+id); });
        $('#folder'+id).attr('rel','exanded');        
        $('#folder'+id).attr('src',$('#folder'+id).attr('src').replace('plustop','minustop'));
        $('#folder'+id).attr('src',$('#folder'+id).attr('src').replace('plusbottom','minusbottom'));
        $('#folder'+id).attr('src',$('#folder'+id).attr('src').replace('plus','minus'));
        
        $.ajax({type: "POST",
          url: '<?php echo url_for("app/setCollapsedTasks?projects_id=" . $sf_request->getParameter("projects_id") )?>',
          data: {tasks_id:id,status:'exanded'}           
        });
      }
      else
      {
        $( ".taskRow"+id ).each(function() { $(this).addClass('hiddenRow'+id); });        
        $('#folder'+id).attr('rel','collapsed');
        $('#folder'+id).attr('src',$('#folder'+id).attr('src').replace('minustop','plustop'));
        $('#folder'+id).attr('src',$('#folder'+id).attr('src').replace('minusbottom','plusbottom'));
        $('#folder'+id).attr('src',$('#folder'+id).attr('src').replace('minus','plus'));
        
        $.ajax({type: "POST",
          url: '<?php echo url_for("app/setCollapsedTasks?projects_id=" . $sf_request->getParameter("projects_id") )?>',
          data: {tasks_id:id,status:'collapsed'}           
        });
      }    
  }
  
  function checkCollapsedTasks()
  {
     $( ".tasksTreePlus").each(function() { 
        
        if($(this).attr('rel')=='collapsed')
        {
          id = $(this).attr('id').replace('folder','')
...
<style> <?php foreach($tasks_tree as $v){ echo ' .hiddenRow' . $v['id']. '{ display:none }'; } ?> </style>
