...
<?php include_component('tasks','childTasks',array('tasks_id'=>$tasks->getId(),'is_email'=>isset($is_email))) ?>
...
<table class="table table-bordered table-hover table-item-details">
  ...
  <?php echo ExtraFieldsList::renderInfoFileds('tasks',$tasks,$sf_user,$tasks->getTasksTypes()->getExtraFields(),array('est_time'=>$tasks->getEstimatedTime(),'work_hours'=>$tasks->getWorkHours()),$is_email) ?>
  
  <?php if($tasks->getEstimatedTime()>0) echo '<tr><th>' . __('Estimated Time') . ':</th><td>' . $tasks->getEstimatedTime() . '</td></tr>';?>
  <?php if($tasks->getWorkHours()>0){ $discrepancy = $tasks->getEstimatedTime()-$tasks->getWorkHours(); echo '<tr><th>' . __('Work Hours') . ':</th><td>' . $tasks->getWorkHours() . '</td></tr>'; } ?>
  <?php //if($tasks->getTogoHours()>0) echo '<tr><th>' . __('Hours To Go') . ':</th><td>' . $tasks->getTogoHours() . '</td></tr>';?>
  <?php //if($tasks->getEstimatedTime()>0 and $tasks->getTogoHours()>0 and $tasks->getWorkHours()>0) echo '<tr><th>' . __('Actual Time') . ':</th><td>' . /*($tasks->getEstimatedTime()-$tasks->getWorkHours()+$tasks->getTogoHours())*/  ($tasks->getEstimatedTime()-$tasks->getWorkHours()). '</td></tr>';?>
  <?php //if($tasks->getTogoHours()==0 and $tasks->getWorkHours()>0) echo '<tr><th>' . __('Actual Time') . ':</th><td>' . $tasks->getWorkHours() . '</td></tr>';?>
  <?php echo '<tr><th>' . __('Actual Time') . ':</th><td>' . /*($tasks->getEstimatedTime()-$tasks->getWorkHours()+$tasks->getTogoHours())*/  ($tasks->getEstimatedTime()-$tasks->getWorkHours()). '</td></tr>'; ?>
...
<?php
  ...
  {
    echo  renderUserPhoto($tasks->getUsers()->getPhoto(),array('width'=>'28','style'=>'width:28px; margin-bottom: 2px;'))  . '&nbsp;' . $tasks->getUsers()->getName() ;
  }   
?>    
    </td>
  </tr> 
