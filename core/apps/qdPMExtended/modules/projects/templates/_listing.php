<?php
  ...
  $extra_fields = ExtraFieldsList::getFieldsByType('projects',$sf_user,true,array('all'=>true));
  $extra_fields_all = ExtraFieldsList::getFieldsByType('projects',$sf_user,false,array('all'=>true));
  $totals = array();
  
  $is_filter = array();
  $is_filter['priority'] = app::countItemsByTable('ProjectsPriority');
  $is_filter['status'] = app::countItemsByTable('ProjectsStatus');
  $is_filter['type'] = app::countItemsByTable('ProjectsTypes');
  $is_filter['group'] = app::countItemsByTable('ProjectsGroups');
...
<table class="table table-striped table-bordered table-hover projects-table"  id="itmes_listing_<?php echo $tlId ?>" <?php echo (count($projects_list)>0 ? 'style="display:none"':'')?> >
  ...
  <tbody>  
    ...
    <tr>     
      ...
      <?php if($is_filter['priority']): ?>
      <td><?php echo app::getArrayNameWithIcon($projects,'ProjectsPriority') ?></td>
      <?php endif; ?>
      
	   <?php if($is_filter['group']): ?>
      <td><?php echo app::getArrayName($projects,'ProjectsGroups') ?></td>
      <?php endif; ?>
	  
	   <?php if($is_filter['type']): ?>
      <td><?php echo app::getArrayNameWithBg($projects,'ProjectsTypes') ?></td>
      <?php endif; ?>
	  
      <?php if($is_filter['status']): ?>
      <td><?php echo app::getArrayNameWithBg($projects,'ProjectsStatus') ?></td>
      <?php endif; ?>
      ...
      <?php
        $v = ExtraFieldsList::getValuesList($extra_fields_all,$projects['id']); 
