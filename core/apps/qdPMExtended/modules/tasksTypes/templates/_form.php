...
<form class="form-horizontal"  id="tasksTypes" action="<?php echo url_for('tasksTypes/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php include_partial('global/formValidator',array('form_id'=>'tasksTypes')); ?>
