...
<form class="form-horizontal" id="discussionsTypes" action="<?php echo url_for('discussionsTypes/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<div class="tab-content" >
    ...
    <div class="tab-pane fade" id="tab_extra_fields">
      ...
      <table>
      <tr>
        <th><?php echo $form['extra_fields']->renderLabel() ?></th>
        <td>
          ...
          <?php if(strlen((string)$form['extra_fields'])>0) echo '<ul class="checkbox_list"><li>' . input_checkbox_tag('set_off_extra_fields',1, array('onClick'=>'set_off_extra_fields_list()','checked'=>($form['extra_fields']->getValue()==array('set_off_extra_fields') ? true:false))) . ' <label for="set_off_extra_fields">' . __('Don\'t use extra fields') . '</label></li></ul>'; ?>
...
<?php include_partial('global/formValidator',array('form_id'=>'discussionsTypes')); ?>