  ...
  <div class="table-scrollable">
    ...
    <thead>
      <tr>
        ...
        <!--<th><div><?php //echo __('To Go'); ?></div></th>-->      
...
<?php
  ...
  $togo_hours = $tasks->getTogoHours(); 
  $est_time = $tasks->getEstimatedTime();
  $actual_time  = $tasks_work_hours[$tasks->getId()]; 
  $actual_time_diff_str = '';
    
  $actual_hours = $est_time-$actual_time+$togo_hours;
  
  if($togo_hours==0 and $actual_time>0) $actual_hours = $actual_time;
  
  $total_est_time += $est_time;
  $total_actual_time += $actual_time;
  
  
  $total_togo_hours += $togo_hours;
...
?>
  <tr>
	...
	<td><?php echo $actual_hours; ?></td>
    <td><?php echo $actual_time_diff_str; ?></td>
   <!-- <td><?php //echo $togo_hours; ?></td>-->  
...
<form action="<?php echo url_for('timeReport/export')?>" method="post" id="tasks_export">
  <?php echo input_hidden_tag('filename','tasks').input_hidden_tag('format').input_hidden_tag('export',json_encode($export)) ?>
</form> 
