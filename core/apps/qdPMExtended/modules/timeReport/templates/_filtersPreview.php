...
?>    
    <li class="list-group-item filter-preview-item">
      <div class="filterPreviewBox">
        <table>
          <tr>
            <td valign="top" style="padding-top: 2px;"><?php echo link_to(image_tag('icons/close_gray.png'),'timeReport/' . $action_name . '?remove_filter=' . $table . ($params? '&' . $params:''),array('title'=>__('Remove Filter')))?></td>
            <td valign="top"><div id="filtersPreviewMenuBox">
            <?php 
            
             if(strstr($table,'extraField'))
             {
               $m = app::getFilterExtraFields(array(),$table,'tasks','timeReport/' . $action_name,false,$filter_by[$table],$sf_user);
             }
             else
             {             
                switch($table)
                {
                  ...
                  case 'Projects':  $m =  app::getFilterProjects(array(),'timeReport/' . $action_name,$params,$filter_by[$table],$sf_user); 
                    break;
                  case 'ProjectsStatus':  $m =  app::getFilterMenuStatusItemsByTable(array(),$table,'Project Status','timeReport/' . $action_name,$params,$filter_by[$table]);
                    break;
