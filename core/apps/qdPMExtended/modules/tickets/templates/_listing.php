<?php
  ...
  $extra_fields = ExtraFieldsList::getFieldsByType('tickets',$sf_user,true,array('all'=>true));
  $extra_fields_all = ExtraFieldsList::getFieldsByType('tickets',$sf_user,false,array('all'=>true));
  $totals = array();  
  $is_filter = array();
  $is_filter['priority'] = app::countItemsByTable('TicketsPriority');
  $is_filter['group'] = app::countItemsByTable('TicketsGroups');
    ...
    <td>
      <table>
        <tr>
          ...
          <td hidden style="padding-right: 15px;"><?php  echo $lc->insert_button(__('Add Ticket')) ?></td>
          <?php endif ?>
	...
	<table class="table table-striped table-bordered table-hover tickets-table" id="itmes_listing_<?php echo $tlId ?>"  <?php echo (count($tickets_list)>0 ? 'style="display:none"':'')?> >
		<thead>
			<tr>
				...
				<th data-bsType="numeric" hidden><div><?php //echo __('Id') ?></div></th>
      			<?php if($is_filter['priority']): ?>
				<th hidden><div><?php //echo __('Priority') ?></div></th>
				<?php endif; ?>	  
				...
				<th><div>Domaine DSI<?php //echo __('Department') ?></div></th>				  
				<th><div>Responsable DSI<?php //echo __('Created By') ?></div></th>                    
				<?php echo ExtraFieldsList::renderListingThead($extra_fields) ?>	  
				<?php if($is_filter['type']): ?>
				<th><div><?php echo __('Type') ?></div></th>
				<?php endif; ?> 
				<?php if($is_filter['group']): ?>                
		...
		<tbody>
			<?php foreach ($tickets_list as $tickets): ?>
				<tr>
					...
					<td hidden><?php //echo $tickets['id'] ?></td>				  
					<?php if($is_filter['priority']): ?>
					<td hidden><?php //echo app::getArrayNameWithIcon($tickets, 'TicketsPriority') ?></td>
					<?php endif; ?>
					<?php if(!$sf_request->hasParameter('projects_id')): ?>
					<td><?php if($tickets['projects_id']>0)echo app::getArrayName($tickets,'Projects')//echo link_to(app::getArrayName($tickets,'Projects'),'projectsComments/index?projects_id=' . $tickets['projects_id']) ?></td>
					<?php endif; ?>				  
					<td>
					...
					<td><?php echo $tickets['Users']['name']; //if(strlen($tickets['user_name'])>0){echo $tickets['user_name']; }else{ echo $tickets['Users']['name']; } ?></td>																 
					<?php
					$v = ExtraFieldsList::getValuesList($extra_fields_all,$tickets['id']); 
					...
					<?php endif; ?> 
					<?php if($is_filter['group']): ?>
					<td><?php echo app::getArrayName($tickets,'TicketsGroups') ?></td>
					<?php endif; ?>
				</tr>
			<?php endforeach; ?>        
		</tbody>  
