...
<form class="form-horizontal" id="tasksLabels" action="<?php echo url_for('tasksLabels/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php include_partial('global/formValidator',array('form_id'=>'tasksLabels')); ?>