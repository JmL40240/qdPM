...
<?php $form->setDefault('tasks_columns_list',explode(',', $form['tasks_columns_list']->getValue())); ?>

<form class="form-horizontal" id="projectsTypes" action="<?php echo url_for('projectsTypes/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<ul class="nav nav-tabs">
  ...
  <li>
    <a href="#tab_tasks_columns" data-toggle="tab"><?php echo __('Tasks Columns') ?></a>
  </li>
</ul>
...
<div class="tab-content" >
    ...
    <div class="tab-pane fade" id="tab_extra_fields">
      <p><?php echo __('Select extra fields which will be appear for this type.') ?></p>
      <table>
      <tr>
        ...
        <td>
          ...
          <?php if(strlen((string)$form['extra_fields'])>0) echo '<div>' . input_checkbox_tag('set_off_extra_fields',1, array('onClick'=>'set_off_extra_fields_list()','checked'=>($form['extra_fields']->getValue()==array('set_off_extra_fields') ? true:false))) . ' <label for="set_off_extra_fields">' . __('Don\'t use extra fields') . '</label></div>'; ?>
        </td>
      </tr>
      </table>
    </div>
    
    <div class="tab-pane fade" id="tab_tasks_columns">
      <p><?php echo __('Select Task fields which will be display in tasks listing for this Project Type.') ?></p>
      <div class="form-group">
      	<label class="col-md-3 control-label"><span class="required">*</span> <?php echo $form['tasks_columns_list']->renderLabel() ?></label>
      	<div class="col-md-9">
      		<?php echo $form['tasks_columns_list'] ?>
      	</div>
      </div>
    </div>
</div>    
