...
<table class="table table-striped table-bordered table-hover" id="table-users">
  <thead>
    <tr>  
      <th data-bSortable="false"  style="width: 20px;"><input name="multiple_selected_all" id="multiple_selected_all" class="group-checkable" data-set="#table-users .checkboxes" type="checkbox"></th>  
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><?php echo ($sf_user->getAttribute('id')!= $users->getId() ? $lc->delete_button($users->getId()):'') . ' ' . $lc->edit_button($users->getId()) ?>
      
      <?php if($users->getActive()==1): ?>
      <a title="<?php echo __('Login') ?>" href="<?php echo url_for('users/loginAsUser?id=' . $users->getId()) ?>" class="btn btn-default btn-xs purple" ><i class="fa fa-sign-in"></i></a>
      <?php endif ?>
