<?php 
  ...
  echo '<div id="dashboard-csg-box">' . button_tag_modalbox(__('Configure Dashboard'),'dashboard/configure') . (count($reports)>1 ? ' ' .  button_tag_modalbox(__('Sort Reports'),'dashboard/sortReports?reports=' . implode(',',$reports)):'') . '</div>';
  ...
  foreach($reports as $report)
  {
    ...
    switch(true)
    {
      case strstr($report,'personalEvents'):          
          include_component('scheduler','viewReport',array('users_id'=>$sf_user->getAttribute('id')));           
        break;
      case strstr($report,'publicEvents'):            
          include_component('scheduler','viewReport',array('users_id'=>0));           
        break; 
