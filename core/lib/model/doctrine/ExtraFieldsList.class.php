...
class ExtraFieldsList extends BaseExtraFieldsList
{
  ...
  public static function getFieldsByType($bind_type,$sf_user,$display_in_list=false, $query_params = array())
  {
    ...
    {
      $q->addWhere('extra_fields_tabs_id=?',$query_params['tab']);
    }
    elseif(!isset($query_params['all']))
    {
      $q->addWhere('extra_fields_tabs_id is null');
    }
    
                
    return $q->orderBy('extra_fields_tabs_id, sort_order, name')                
                ->fetchArray();                                
  }
    ...
    { 
      ...
      if($ef['type']=='file')
      {
        ...
        if(strlen($files['extra_fields'][$ef['id']]['name'])>0)
        {
          ...
          $a->setBindId($ef['id']);
          $a->setUsersId($sf_user->getAttribute('id'));
          $a->setDateAdded(time());
          $a->save();
  ...
  {  
    ...
    foreach($extra_fields as $v)
    {
      if($v['display_in_list']==1)
      {
        ...
        {
          case 'number':
              $attr = 'data-bsType="numeric"';
            break;
        }
        
        $html .= '<th ' . $attr . '><div>' . $v['name'] . '</div></th>';
      } 
    }
  ...
  {  
    ...
    foreach($extra_fields as $v)
    {
      if($v['display_in_list']==1)
      {
        $html .= '<td>' . ExtraFieldsList::renderFieldValueByType($v,$values,$extra_values,false,true) . '</td>';
      }
    }
  ...
  public static function getListingTotals($totals, $extra_fields,$values,$extra_values = array())
  {
    foreach($extra_fields as $v)
    {
      if($v['display_in_list']==1 and in_array($v['type'],array('number','formula')))
      {
        ...
        $value = ExtraFieldsList::renderFieldValueByType($v,$values,$extra_values);
  ...
  public static function renderInfoFileds($bind_type,$object,$sf_user, $use_fields = '',$extra_values = array(),$is_email = false)
  {                      
    if(strlen($use_fields)>0 and $use_fields=='set_off_extra_fields') return '';
    ...
    foreach($extra_fields as $v)
    {
      ...
      if($is_email and strlen($v['users_groups_access'])) continue;        
    ...
    $html .= ExtraFieldsTabs::renderTabsInDetailsBox($bind_type,$object,$sf_user, $use_fields ,$extra_values);
  ...
  public static function renderFormFileds($bind_type,$object,$sf_user, $extra_fields = null)
  {
    ...
    foreach($extra_fields as $v)
    {
      if($v['type']=='formula' or in_array($sf_user->getAttribute('users_group_id'),explode(',',$v['view_only_access']))) continue;
                  
      $html .='
      <div class="form-group extra_field_row" id="extra_field_row_' . $v['id'] . '">
      	<label class="col-md-3 control-label">'  . ($v['is_required']==1 ? '<span class="required">*</span> ':'') . $v['name'] . ($v['type']=='checkbox' ? '<br><a href="#" onClick="return checkAllInContainer(\'checkboxesList' . $v['id']. '\')"><small>' .  __('Select All') . '</small></a>':'').  '</label>
      	<div class="col-md-9">
  ...
  public static function renderFieldValueByType($f,$values,$extra_values = array(),$is_export = false,$is_listing = false)
  {
    ...
    switch($f['type'])
    {
      case 'file':
        ...
        break;
      case 'date_dropdown':
          return (strlen($value)>2 ? app::dateFormat($value):'');
        break;
      case 'date':
          return app::dateFormat($value,0,$is_listing);
        break;
      case 'date_time':
          return app::dateTimeFormat($value,0,$is_listing);
        break;
      case 'date_range':
          if(strlen($value)>1)
          {
            ...
            return  app::dateFormat($value[0],0,$is_listing) . ' - ' . app::dateFormat($value[1]);
          }
          else
          {
            return '';
      ...
      case 'formula':                 
           ...
           if(!strstr($formula,'['))
           {
              ...
              if(strstr($formula,'{'))
              {
                $eval_str = 'function getFormula' . $f['id'] . 'Value()' . $formula . '; $fv=getFormula' . $f['id'] . 'Value();';
              }
              else
              {                  
                $eval_str = '$fv=' . $formula . ';';
  ...
  {    
    ...
    switch($f['type'])
    {
      case 'file':
        ...
        break; 
      case 'number':
          $attributes['class'] = 'form-control input-small' . $required_css;
          return input_tag('extra_fields[' . $f['id'] . ']',$value,$attributes);
        break;        
      case 'text':
      case 'url':
          $attributes['class'] = 'form-control input-large' . $required_css;
          return input_tag('extra_fields[' . $f['id'] . ']',$value,$attributes);
        break;  
      case 'textarea':
          $attributes['class'] = 'form-control input-xlarge' . $required_css;
          return textarea_tag('extra_fields[' . $f['id'] . ']',$value,$attributes);
        break;
      case 'textarea_wysiwyg':
          if(isset($attributes['class']))
          {
            $attributes['class'] .= ' form-control editor' ;
          }
          else
          {
            $attributes['class'] = 'form-control editor';
      ...
      case 'date':
          if(isset($attributes['class']))
          {
            $attributes['class'] .= ' form-control' . $required_css;
          }
          else
          {
            $attributes['class'] = 'form-control ' . $required_css;
      ...
      case 'checkbox':
          ...
          return '<div id="checkboxesList' . $f['id'] . '" class="checkboxesList' .  $f['id'] . '">' . select_tag('extra_fields[' . $f['id'] . ']',explode("\n",$value),array('choices'=>$choices, 'expanded' => true,'multiple' => true),array('class'=>$required_css,'data-checkbox-list'=>$f['id'])) . '</div>';
        break;
      case 'radiobox':
          $choices = array();
          ...
          return '<div class="radio-list radio-list-' . $f['id'] . '">' . select_tag('extra_fields[' . $f['id'] . ']',$value,array('choices'=>$choices, 'expanded' => true),array('class'=>$required_css,'data-raido-list'=>$f['id'])) . '</div>';
        break;
    }
  }  
}