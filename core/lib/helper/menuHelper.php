<?php
  ...
  function renderYuiMenu($id, $m, $html = '', $level=0)
  {
    ...
    {            
      ...
      {
        $html .= '
                <li class="' . ($level==0?'yuimenubaritem':'yuimenuitem') . $selected_class . '"><a ' . (isset($v['confirm'])?'onClick="return confirm(\'' . __('Are you sure?') . '\')"':''). ' class="' . ($level==0?'yuimenubaritemlabel':'yuimenuitemlabel') . '" href="' . (isset($v['url']) ? url_for($v['url']) : '#'). '">' . $v['title']. '</a>';
      }
      else
      {
        $html .= '
  ...
  {
    $menu_cfg_users = array('usersGroups','projectRoles');
    $menu_cfg_projects = array('projectsStatus','projectsTypes','phases','phasesStatus','versionsStatus','projectsGroups','projectsPriority');
    $menu_cfg_tasks = array('tasksStatus','tasksTypes','tasksLabels','tasksPriority');
    $menu_cfg_tickets = array('departments','ticketsStatus','ticketsTypes','ticketsGroups','ticketsPriority');
    $menu_cfg_discussions = array('discussionsStatus','discussionsTypes','discussionsGroups','discussionsPriority');
    $menu_cfg_contacts = array('contactsGroups');
    $menu_cfg_events = array('eventsPriority','eventsTypes');
    $menu_tools = array('backup','xlsTasksImport','trash','msProjectImport');
    $menu_users = array('users','sendEmail');
    $menu_reports = array('projectsReports','userReports','ticketsReports','discussionsReports','timeReport','ganttChart','usersActivity','usersProjectsReport','usersTasksReport');
    ...
    elseif($moduleName=='configuration' and in_array($pageData['moduleName'],array_merge($menu_cfg_users,$menu_cfg_projects,$menu_cfg_tasks,$menu_cfg_tickets,$menu_cfg_discussions,$menu_cfg_contacts,$menu_cfg_events,array('extraFields','commonReports'))) and $level==0)
    {
      return true;
    }
    elseif(in_array($moduleName,$menu_cfg_users) and $level==1 and (in_array($pageData['moduleName'],$menu_cfg_users) or $pageData['sf_request']->getParameter('t')=='UsersGroups'))
    {            
      return true;
    }
    elseif(in_array($moduleName,$menu_cfg_projects) and $level==1 and (in_array($pageData['moduleName'],$menu_cfg_projects)  or $pageData['sf_request']->getParameter('t')=='ProjectsTypes'))
    {      
      return true;
    }
    elseif(in_array($moduleName,$menu_cfg_tasks) and $level==1 and (in_array($pageData['moduleName'],$menu_cfg_tasks) or $pageData['sf_request']->getParameter('t')=='TasksTypes'))
    {      
      return true;
    }
    elseif(in_array($moduleName,$menu_cfg_tickets) and $level==1 and (in_array($pageData['moduleName'],$menu_cfg_tickets) or $pageData['sf_request']->getParameter('t')=='TicketsTypes'))
    {      
      return true;
    }
    elseif(in_array($moduleName,$menu_cfg_discussions) and $level==1 and (in_array($pageData['moduleName'],$menu_cfg_discussions) or $pageData['sf_request']->getParameter('t')=='DiscussionsTypes'))
    {      
      return true;
    }
    elseif(in_array($moduleName,$menu_cfg_contacts) and $level==1 and in_array($pageData['moduleName'],$menu_cfg_contacts) )
    {      
      return true;
    }
    elseif(in_array($moduleName,$menu_cfg_events) and $level==1 and in_array($pageData['moduleName'],$menu_cfg_events) )
    {      
      return true;
    }
    ...
    elseif($pageData['sf_request']->getParameter('bind_type')=='contacts' and $moduleName=='contactsGroups' and $level==1)
    {
      return true;
    }  
    elseif($pageData['sf_request']->getParameter('bind_type')=='events' and $moduleName=='eventsPriority' and $level==1)
    {
      return true;
    }   
    ...
    elseif($pageData['sf_request']->hasParameter('t') and $actionName=='extraFieldsByGroup')
    {      
      
      if(strstr($url,'t=' . $pageData['sf_request']->getParameter('t')))
      {
        return true;
      }          
    }   
  ...
  { 
    ...
    foreach($menu as $v)
    {
      ...
      $is_active = isset($v['url']) ? isSidebarMenuItemActive($v['url'],$currentPageData,$level) : false;
      ...
      $html .= '
        <a ' . ($v['target'] ? 'target="' . $v['target'] . '"':''). ' ' . $url . '>' . (isset($v['class']) ? '<i class="fa ' . $v['class'] . '"></i> ':'') . (isset($v['counts']) ? ' <span class="badge badge-info">' . $v['counts'] . '</span>':'')  . '<span class="title">' . $v['title'] . '</span>' . (isset($v['submenu']) ? '<span class="arrow ' . ($is_active ? 'open':'') . '"></span>':''). '</a>';
