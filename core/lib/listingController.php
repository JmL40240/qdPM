...
class listingController
{
  ...
  public function add_child_button($id,$projects_id=0)
	{
	  if($this->access['insert'])
	  {
	    ...
	    //$attributes = array('type'=>'image','title'=>__('Add Child Task'),'class'=>'actionIcon iconTree','src'=>public_path('images/icons/add_child_task.png',true),'onClick'=>'openModalBox(\'' . url_for($this->module . '/new?parent_id=' . $id  . $this->add_projects_id($projects_id) . $this->add_url_params('&'),true) . '\')');
      //return $f->render('',__('Edit'),$attributes);
      
      return '<a title="' . __('Add Child Task') . '" href="#" class="btn btn-default btn-xs purple" onClick="openModalBox(\'' . url_for($this->module . '/new?parent_id=' . $id  . $this->add_projects_id($projects_id) . $this->add_url_params('&'),true) . '\'); return false;"><i class="fa fa-sitemap"></i></a>';
    }
  }
  ...
  public function sort_tasks_tree($title)
	{
	  ...
	  if($this->access['edit'])
	  {      
      return '<button class="btn btn-primary" onClick="location.href=\'' . url_for($this->module . '/sortTree' . $this->add_url_params('?'),true) . '\'">' . $title . '</button>';
    }
  }
  ...
  {
    return '<th data-bSortable="false" class="table-checkbox"><input class="group-checkable" data-set="#' . $table_id . ' .checkboxes"  type="checkbox"></th>';
  }
  ...
  {  
    ...
    if($this->access['edit'])
    {
      ...
      {        
        $s[] = array('title'=>__('Copy To'),'url'=>$this->module . '/copyTo' . $this->add_url_params('?'),'mamodalbox'=>true);
        
        /*
        if($this->sf_request->hasParameter('projects_id'))
        {
          $s[] = array('title'=>__('MS Project Export'),'url'=>$this->module . '/msProjectExport' . $this->add_url_params('?'),'mamodalbox'=>true);
        }
        */
      }
    }    
